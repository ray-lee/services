...
public class CreateCoreCatalogFromMedia implements BatchInvocable
{
  ...
  public CreateCoreCatalogFromMedia()
  {
    trace( "Initialized at " + new Date().toString() );
    CreateCoreCatalogFromMedia.setupClassStatics();
    context = null;
  ...
  /**
   * @return a non-empty set of modes supported by this plugin.
   * First method called after invocation
   */
  @Override
  public List<String> getSupportedInvocationModes()
  {
    trace( "In getSupportedInvocationModes" );
    return CreateCoreCatalogFromMedia.invocationModes;
  }

  /**
   * Sets the invocation context for the batch job.
   * Second method called after invocation, before run().
   * 
   * @param context
  ...
  public void run()
  {
    ...
    if (res.getStatus() == Response.Status.OK.getStatusCode())
    {
        ...
        trace( "Error reading media record. Status code = " + res.getStatus());
    ...
    String mediaXML = fetchMediaXML( context.getSingleCSID() );
    trace( "mediaXML: " + ((mediaXML == null) ? "No xml document" : mediaXML ));
    trace( "MEDIAX DONE" );
    ...
    if( createCatalogRecord( mediaInfo ) == STATUS_COMPLETE )
    {
     ...
     if( createRelation( catalogId, mediaId, RELATION_TYPE ) ==
     STATUS_COMPLETE )
     {
    ...
    trace( "CATALOG PAYLOAD: " +
           (( catalogPayload == null ) ? "No catalog payload" : catalogPayload ));
  ...
  private String fetchMediaXML( String csid )
  {
    ...
    String userpass = "admin@core.collectionspace.org:Administrator";
  ...
  private int createRelation( String catalogId, String mediaId, String relType )
  {
    ...
    String payload1 = createRelationPayload( catalogId, catalogType, mediaId,
        mediaType, relType );
  ...
  /**
   * Sets the invocation context for the batch job.
   * Third method called after invocation, before run().
   * 
   * @param context
   *          an instance of InvocationContext.
   */
  ...
  private void printContextInfo()
  {
    ...
    else
    {
      ...
      if( params != null )
      {
        List<InvocationContext.Params.Param> paramList = params.getParam();
      ...
      // String statusMsg = "Catalog and Relation creation failed.";
      //
      // if( createCatalogRecord( "MH-10030", "My Object" ) == STATUS_COMPLETE )
      // {
      // statusMsg = "Catalog created successfully.";
  ...
  private class BatchJobParserHandler extends DefaultHandler
  {
    ...
    {
      trace("Start element: name=" + localName + 
            ", qName=" + qName + ", uri=" + uri);
      if( qName.equalsIgnoreCase( "ns2:media_common" ))
        ...
        inUcjeps = true;
      else if(inCommon && ( qName.equalsIgnoreCase( "identificationNumber" )))
    ...
    {
      trace( "End element: name="  + localName +
             " qName: " + qName + " uri: " + uri);
      if( qName.equalsIgnoreCase( "ns2:media_common" ))
        inCommon = false;
